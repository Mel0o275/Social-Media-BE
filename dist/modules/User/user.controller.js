...
router.post("/logout", (0, auth_middelware_1.authentication)(), async (req, res) => {
    try {
        ...
        const result = await user_service_1.authSecurityService.logout(decoded);
...
router.patch("/updatePassword", (0, auth_middelware_1.authentication)(), async (req, res) => {
    try {
        const user = req.user;
        const result = await user_service_1.authSecurityService.updatePass(user, req.body);
...
async (req, res) => {
    ...
    try {
        // const image = await authSecurityService.profileImage(req.user, req.file as Express.Multer.File);
        const image = await user_service_1.authSecurityService.profileImage(req.body, req.user);
...
router.patch("/cover-images", (0, auth_middelware_1.authentication)(), (0, multer_1.cloudFileUpload)({
    ...
    try {
        ...
        const images = await user_service_1.authSecurityService.coverImage(req.user, req.files);
...
router.delete("/delete-profile", (0, auth_middelware_1.authentication)(), async (req, res) => {
    try {
        const result = await user_service_1.authSecurityService.deleteProfile(req.user);
        res.status(200).json(result);
    }
    catch (error) {
