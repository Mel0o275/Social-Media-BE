...
router.get('/user/:userId', (0, auth_middelware_1.authentication)(), async (req, res) => {
...
router.get('/:postId', (0, auth_middelware_1.authentication)(), async (req, res) => {
    const postId = Array.isArray(req.params.postId)
        ? req.params.postId[0]
        : req.params.postId;
    const data = await post_service_1.postService.getPostById(new mongoose_1.Types.ObjectId(postId), req.user);
    return res.status(200).json({
...
router.get('/:postId/likes', (0, auth_middelware_1.authentication)(), async (req, res) => {
    const postId = Array.isArray(req.params.postId) ? req.params.postId[0] : req.params.postId;
    const data = await post_service_1.postService.getPostReactions(new mongoose_1.Types.ObjectId(postId), req.user);
    return res.status(200).json({
